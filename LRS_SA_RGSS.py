# -*- coding: utf-8 -*-
"""
Created on Thu May  3 10:35:53 2018
    ...
    def validation(self, selectcol, num, addfeature, coetest=0):#get the score with the new features list and update the best features combination
        ...
        if 1:
            ...
            for train_index,test_index in sfolder.split(X,y):
                ...
                totaltest += self.LossFunction(y_test, self.clf.predict_proba(X_test,num_iteration=self.clf.best_iteration_)[:,1])
            totaltest /= 5.0
...
#            print('Mean loss: {}'.format(totaltest))
    ...
    def MyRandom(self):
        ...
        for i in self.bestfeature:
            col.remove(i)
        for t in range(5,10): # add 4 to 8 features randomly, choose your own range
            print('add {} features'.format(t))
            for i in range(10): # run 50 rounds each quantity, choose your own round number
